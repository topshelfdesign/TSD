...
//        wp_enqueue_style( 'app', NV_CSS.'/app.css' );
		...
		// TSD Custom CSS
		wp_enqueue_style('foundation', NV_CSS . '/foundation.css');
		wp_enqueue_style('custom', NV_CSS . '/custom.css');
		wp_enqueue_style('responsive', NV_CSS . '/responsive.css');
		...
		wp_enqueue_script('isotope', '//cdnjs.cloudflare.com/ajax/libs/jquery.isotope/2.0.0/isotope.pkgd.min.js');
		wp_enqueue_script('waitforimages', '//cdnjs.cloudflare.com/ajax/libs/jquery.waitforimages/1.5.0/jquery.waitforimages.js');
		...
		wp_enqueue_script(
			'cycle2',                                    // uid
			'//cdnjs.cloudflare.com/ajax/libs/jquery.cycle2/20140415/jquery.cycle2.min.js',                            // url
			array('jquery', 'foundation', 'modernizr'),   // dependencies (by uid)
			false,                                          // version id (optional)
			true                                            // load in footer?
		);
		...
		wp_enqueue_script(
			'cycle2_center',                                    // uid
			'//cdnjs.cloudflare.com/ajax/libs/jquery.cycle2/20140415/jquery.cycle2.center.min.js',                            // url
			array('jquery', 'foundation', 'modernizr'),   // dependencies (by uid)
			false,                                          // version id (optional)
			true                                            // load in footer?
		);
		...
		wp_enqueue_script(
			'custom_portfolio',                                                                                // uid
			NV_JS . '/custom-portfolio-individual.js',                                                    // url
			array('jquery', 'foundation', 'modernizr'),   // dependencies (by uid)
			false,                                          // version id (optional)
			true                                            // load in footer?
		);

		wp_enqueue_script('tsd_js', NV_JS . '/tsd_custom.js', array('jquery', 'foundation', 'modernizr'),false,true);
		wp_enqueue_script('tsd_js_ajax', NV_JS . '/tsd_wp_api_calls.js', array('jquery', 'foundation', 'modernizr'),false,true);
		wp_enqueue_script('maskedinput', NV_JS . '/jquery.maskedinput.min.js', array('jquery'), false, true);
	}
